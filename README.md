...
- **功能**：
  ...
  - 未填写 `save_path` 时自动使用 ComfyUI 实际 output 目录，而不是写死的相对路径
  - 留空 `save_path` 时会在节点上显示默认 output 目录中的图像预览
  - 填写 `save_path` 时会仅保存到该目录，并在节点上返回保存路径文本
  - 只保存 JPG
  - 自动生成唯一文件名，格式：`前缀+时间戳+索引.jpg`，时间戳精确到微秒，避免同秒重复运行时文件名重复
  - 支持批量处理多张图像
...
1. 下载或克隆此仓库
2. 将`KOOK_ImageCompression`文件夹复制到ComfyUI的`custom_nodes`目录下
3. 重启ComfyUI
4. 在节点菜单中找到"image"分类下的节点，或搜索"图片压缩"、"JPG"
